...
export default function ModalEdit({open, setOpen, fields, reload, data}) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if(response){
        ...
        const edited = resp.map(dt => {
          if(dt.id === data.id){
            return forms
          } dt
          return
        })
        localStorage.setItem('companies', JSON.stringify(edited))
      }
      setOpen(false)
      await reload()
    } catch (error) {
                    ...
                    <form className="mb-2 w-full">
                      {
                        fields.map((field, i) => {
                          ...
                          <div className="mb-4" key={i}>
                            ...
                            <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id={field.name} type="text" placeholder={field.label} onChange={(e) => handleChange(e.target.value, field.name)} value={forms[field.name]}/>
                          </div> :
                           <div className="mb-4" key={i}>
                           <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor={field.name}>
                             {field.label}
                           ...
                           <div className="relative">
                              <select className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state" onChange={(e) => handleChange(e.target.value, field.name)}>
                                ...
                                {
                                  field.options.map((option, o) => <option value={option.value} key={o} selected={forms[field.name] === option.value}>{option.name}</option>)
                                }
                              </select>
